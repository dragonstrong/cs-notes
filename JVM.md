...
<img src="assets/image-20240413223258813.png" alt="image-20240413223258813" style="zoom:50%;" />
...
辩证看待jdk演进中内存区域的调整，下图仅为通用图：

<img src="assets/image-20240409005300446.png" alt="image-20240409005300446" style="zoom:80%;" />



![image-20240409122122749](assets/image-20240409122122749.png)
...
- 局部变量表：存基本数据类型、对象引用
- 局部变量表所需内存大小在编译期间确定，进入一个方法时，它所需要在帧栈中分配多大的局部变量空间时完全确定的，不会在方法运行期间改变大小。
...
<img src="assets/image-20240409142259551.png" alt="image-20240409142259551" style="zoom:80%;" />
...
- 起始PC：字节码指令的行号
- 长度：与起始PC合起来标示当前变量的作用域（比如变量test，其作用域起始位置对应字节码行号为8，作用域为从起始位置往后数12行字节码，可以发现起始PC+长度为定值20，即main方法字节码的长度）

![image-20240409153128279](assets/image-20240409153128279.png)



局部变量和类变量：类变量在类加载过程中会赋默认值，而==局部变量没有初始化过程，必须人为初始化，否则报错：==

```
public void test(){
    int i;
    System.out.println(i); // 错误，没有赋值不能使用
}
```
...
- 执行指令0，将15放入操作数栈
- 执行指令2，将15从操作数栈弹出存入局部变量表下标0位置 （其中istore_0表示存入下标0）
- 执行指令3，将8放入操作数栈
- 执行指令5，将8从操作数栈弹出存入局部变量表下标1位置
- 执行指令6，取出局部变量表下标0处的15放入操作数栈
- 执行指令7，取出局部变量表下标1处的8放入操作数栈
- 执行指令8，将15和8从操作数栈弹出给执行引擎CPU计算结果，结果23保存到操作数栈
- 执行指令9，将23从操作数栈弹出保存到局部变量表下标2位置
- 执行指令10，退出方法返回
...
- 存放所有类的对象实例、数组（数组也是对象）。jdk1.7及以后将方法区中的静态变量（类级别）和字符串常量池也移到堆中。

- ==**对象和数组永远不会存储在栈上，因为栈帧中保存引用，这个引用指向对象或者数组在堆中的位置**==。
...
4. 后面重复上面的Minor GC，到图3的特殊情况，判断s1中的有用对象时发现有些年龄到了15，此时将它们（年龄为15的对象）移到老年区（晋升）
5. 若s0/s1清理无用对象还满了，那么s0/s1中的对象直接晋升到老年代，不受年龄15的限制
...
- 存储已被虚拟机加载的 ==<font color=red>**类型信息、常量（运行时常量池）、静态变量(jdk1.7之前)**</font>==、编译后的代码缓存等
...
永久代回收效率很低，在Full GC（老年代和永久代空间不足）时才触发。而开发中会有大量字符串创建，导致字符串常量池无法及时被回收，从而引发永久代内存不足。
...
- 静态变量和类关联在一起，随类的加载而记载，成为类数据在逻辑上的一部分

- 类变量（静态变量）被类的所有实例共享，即使没有类实例也能访问。
...
静态变量（符号引用）存放在方法区中（jdk1.8移到堆中），若它指向类对象，这个对象实体永远存在堆中。
...
只要不断创建对象，并且保证GC Roots到对象之间有可达路径以避免垃圾回收清除这些对象即可。测试代码如下：
...
- 引用计数：在对象中添加一个引用计数器，每当有一个地方 引用它时，计数器值就加一；当引用失效时，计数器值就减一；任何时刻计数器为零的对象就是不可能再被使用的。缺点：有**循环引用问题-> 内存泄漏**、增加存储开销（单独的字段存储计数器）
...
- **虚拟机栈中引用的对象（当前方法，此时已经运行结束的方法断了引用链，成了垃圾）**
...
1. 初始标记：暂停用户线程（t1），标记处与GC Roots直接相连的对象
2. 并发标记：从GC Roots的直接关联对象开始遍历整个对象图，该过程耗时教程，无需暂停用户线程
3. 重新标记：暂停用户线程(t2)，修正并发标记期间因用户线程运行导致的变动，停顿时间较初始标记长
...
- 相较于CMS，G1还不具备全方位压倒性优势。比如内存占用和程序运行时的额外负载都比CMS高
...
- -XX:InitiatingHeapOccupancyPercent=30  设置触发并发GC周期的java堆占用率阈值，默认值45
