...
2. **<font color=red size=5>类中的静态变量一个类只有一份（jdk1.8存在堆中，1.7以前在方法区），多个实例共享</font>**,  对象本身的内存结构里不存静态变量，任一实例修改静态变量都对其他实例可见。而**<font color=red size=5>非静态成员变量每个实例各有一份（存在方法区）</font>**，修改仅影响自身实例。
...
class Test{
    ...
    private Integer j;  // 非静态成员变量 每个实例都有自己的， 存放在方法区
