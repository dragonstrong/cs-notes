...
```java
final class Singleton{ // 加final：防止有子类破坏单例
...
final class Concurrency{
    ...
    public void actor2(R r){
        num=2;
        ready=true;  // ready上加volatile可以保证它之前的代码重排序，写屏障
    }
}
```
...
在JDK种Boolean, , Integer, Long, Character 等包装类提供了valueOf方法，例如**==Long的valueOf会缓存-128~127之间的Long对象，在这个范围之间会重用对象，大于这个范围，才会新建Long对象==**
...
自己创建的单个Thread任务执行失败了会直接结束，没有任何不久措施，而**==线程池newSingleThreadExecutor还会新建一个线程，维持线程池种线程数量为1，接着会用新创建的线程去执行任务==**，保证池正常工作。
...
executorService.execute(()->{
    log.info("task1: {}",10/0);  // 异常结束，但会再创建一个线程保证池正常工作
});
...
调用ExecutorService中的shutdown方法：线程池状态变为SHUTDOWN，**==不会接收新任务，但已提交的任务会执行完==**，不会阻塞调用线程的执行。
